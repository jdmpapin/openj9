...
KnownObjOperand::KnownObjOperand(
...
Operand*
NullOperand::merge1(Operand* other)
   {
...
void
NullOperand::printToString(TR::StringBuf *buf)
   {
   buf->appendf("(null)");
   }
...
Operand *
InterpreterEmulator::knownObjOperand(TR::KnownObjectTable::Index i, TR_OpaqueClassBlock *clazz)
   {
...
void
InterpreterEmulator::maintainStackForTableSwitch()
   {
   assertHasState();
...
void
InterpreterEmulator::maintainStackForArraylength()
   {
   assertHasState();
   Operand *obj = pop();
...
void
InterpreterEmulator::maintainStackForArrayLoad(TR::DataTypes type, TR_J9ByteCode bc)
   {
   assertHasState();
...
Operand *
InterpreterEmulator::foldArrayLoad(
...
bool
InterpreterEmulator::hasVisitedPred(TR::Block* block)
   {
...
void
InterpreterEmulator::maintainStackForNew(int32_t cpIndex)
   {
   assertHasState();
...
void
InterpreterEmulator::maintainStackForInstanceof(int32_t cpIndex)
   {
   assertHasState();
...
InterpreterEmulator::refineResolvedCalleeForInvokestatic(
   ...
   switch (rm)
      ...
      case TR::java_lang_invoke_MethodHandle_linkToNative:
         ...
         if (knot == NULL
             || knot->isNull(nativeMH)
             || nativeMH == TR::KnownObjectTable::UNKNOWN)
            {
            return;
            }
...
#endif // TR_INLINE_LINKTONATIVE_MH
...
bool
InterpreterEmulator::shouldIterateWithState()
   {
   if (comp()->compileRelocatableCode())
      return false;
...
bool
InterpreterEmulator::isEdgeUnreachable(TR::CFGEdge *edge)
   {
   return _unreachableEdges.count(edge) != 0;
   }

bool
InterpreterEmulator::isBlockUnreachable(TR::Block *block)
   {
   return _blockRc[block] == 0;
   }

void
InterpreterEmulator::markSuccessorsUnreachable(const TR::CFGEdgeList &edges)
   {
   assertHasState();
   auto end = edges.end();
...
void
InterpreterEmulator::markEdgeUnreachable(TR::CFGEdge *edge)
   {
...
void
InterpreterEmulator::markEdgeUnreachable(int32_t destBcIndex)
   {
   markEdgeUnreachable(_currentInlinedBlock->getEdge(blocks(destBcIndex)));
   }

void
InterpreterEmulator::markSweepCFG()
   {
